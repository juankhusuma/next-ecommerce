...
`;
export default async function GetAuthenticatedUser(req: NextApiRequest, res: NextApiResponse) {
    const defaultReturnObject = { authenticated: false, user: null };
    try {
        const token = String(req?.headers?.authorization?.replace('Bearer ', ''));
        const decoded = jwt.verify(token, JWT_SECRET!);
        const getUserResponse = await client.request(getUserByEmailQuery, { email: (decoded as any).email });
        const { nextUser } = getUserResponse as any;
        if (!nextUser) {
