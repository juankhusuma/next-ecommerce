...
export default withIronSessionApiRoute(
    async function signIn(req, res) {
        ...
        const getUserResponse = await client.request<{ nextUser: { email: string, password: string } }>(getUserByEmailQuery, { email });
        ...
        const updateUserResponse = await client.request<{ updateUserResponse: any }>(updateUserMutation, { where: { email }, data: { token } });
        const { updateNextUser } = updateUserResponse as any;
